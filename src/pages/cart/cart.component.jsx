...
import {apiCreateOrderClient, apiGetInfoOrder, apiCheckaddress} from "../../api";
...
class Cart extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            type: "normal",
            search: "",
            total: JSON.parse(localStorage.getItem("location")).price,
            delivery:JSON.parse(localStorage.getItem("location")).price,
            products_search: []
        }
    ...
    componentDidMount() {
        this.setState({addresstoshow: JSON.parse(localStorage.getItem("location")).address})
    ...
    createOrder = (e) => {
        ...
        if (price < 34000) {
            ...
            const {addresstoshow, indicaciones, comentarios,delivery} = this.state;
            const token = localStorage.getItem("token");
                ...
                return (
                    <div key={cart.idProduct} className="d-flex bd-highlight"
                        ...
                        <div className="p-2 bd-highlight">
                            ${cart.price * cart.quant  } <p style={{display: parseInt(cart.package)===0? "none":"inline"}}>{ "+ $" + 500*parseInt(cart.package)*parseInt(cart.quant)}</p>
                        </div>
            ...
            items = data.map((cart, i) => {
                ...
                return (
                    <div key={i} className="d-flex bd-highlight"
                        ...
                        <div className="p-2 bd-highlight">
                            ${cart.productInfo.price * cart.amount} <p style={{display: parseInt(cart.productInfo.package)===0? "none":"inline"}}>{ "+ $" + 500*parseInt(cart.productInfo.package)*parseInt(cart.amount)}</p>
                        </div>
                    </div>
                );
            })
                            ...
                            <Button variant="primary" size="sm" block onClick={this.checkaddress}> Actualizar dirección </Button>
                                ...
                                <Form.Group controlId="exampleForm.ControlTextarea1">
                                    ...
                                    <Form.Control as="textarea" rows="3"
                                                  ...
                                                  readOnly={this.state.type !== "normal"}
                                                  placeholder={"Nombre del conjunto, torre, apartamento"}
                                    />
                                </Form.Group>
                                <Form.Group controlId="exampleForm.ControlTextarea1">
